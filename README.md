...
|-----------|----------------------|
| **Autonomous Execution Quality** | Code-mode self-correction: a wrong call is a concrete runtime error returned to the agent — see [Architecture.md](docs/Architecture.md). Self-correction is visible in the bundled chat transcript. |
| **IR Accuracy** | Typed SDK fails fast on invented methods/fields; findings grounded in structured returns, each cited to an audited command — [Architecture.md](docs/Architecture.md), [AuditTrail.md](docs/AuditTrail.md). |
