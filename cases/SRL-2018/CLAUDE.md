...
Stark Research Labs (SRL) is an enterprise research organization. SRL's security team received intelligence that an
advanced threat actor targeted the company to steal proprietary research - in particular the secret "Carbonadium"
formula. You have been engaged by the SRL CISO to perform enterprise incident response across the affected Windows
estate (a domain controller, two Remote Desktop Services hosts, a file server, and two workstations) and reconstruct
the full scope and narrative of the intrusion.

Disk and memory images were acquired from each host over the network via F-Response (FTK Imager; acquisition case
number 20180905-001; acquired 6-7 September 2018). Treat every image as strictly read-only (chain of custody).

Investigate autonomously to completion and answer the incident-response questions below. Do not pre-judge which host
was hit first or how the attacker moved - establish each point from the evidence. Where the evidence does not support
a confident answer, say so and record the gap rather than guessing.

Questions to be answered:

## 1. Initial access and "System Zero"
* Which system was compromised first ("System Zero" / patient zero)?
* What was the initial-access vector, and when (UTC) did the intrusion begin?

## 2. Lateral movement
* Which systems did the attacker move to, in what order, and when (UTC)?
* How did they move laterally between systems (e.g. RDP, PsExec, WMI, scheduled tasks, remote services)?

## 3. Credential theft and privilege escalation
* What credential-theft tooling or technique was used (e.g. LSASS dumping, mimikatz, Windows Credential Editor)?
* Which accounts were compromised, and did the attacker obtain domain-level (Domain Admin) privileges?

## 4. Malware, tooling and persistence
* What attacker tools, malware, or rogue processes are present on each host?
* What persistence mechanisms were established (services, Run keys, scheduled tasks, WMI event consumers)?

## 5. Data targeting and exfiltration
* What data did the attacker target or stage (notably the "Carbonadium" research)?
* Is there evidence of exfiltration, and by what channel?

## 6. Timeline, scope and anti-forensics
* Produce a consolidated UTC timeline of the intrusion across all hosts.
* What is the full scope of compromise (which hosts and accounts), and is there evidence of anti-forensics
  (event-log clearing, timestomping, USN/MFT tampering)?
...
Six Windows hosts, imaged 6-7 Sep 2018 via FTK Imager / F-Response (acquisition case 20180905-001). The evidence is
split across TWO directories on the SIFT workstation - the domain controller and the two RDS hosts live under
`/mnt/artifacts/srl-2018/`, while the file server and the two workstations live under
`/home/sansforensics/artifacts/srl2018/` (note: no hyphen, and under `artifacts/`).

| Image | Host (role) | Kind |
|-------|-------------|------|
| `/mnt/artifacts/srl-2018/base-dc-cdrive.E01` | dc01 (domain controller) | Disk image (NTFS C:, EWF/E01) |
| `/mnt/artifacts/srl-2018/base-dc-memory.img` | dc01 (domain controller) | Memory capture (raw) |
| `/mnt/artifacts/srl-2018/base-rd-01-cdrive.E01` | rd-01 (Remote Desktop Services host) | Disk image (NTFS C:, EWF/E01) |
| `/mnt/artifacts/srl-2018/base-rd01-memory.img` | rd-01 (Remote Desktop Services host) | Memory capture (raw) |
| `/mnt/artifacts/srl-2018/base-rd-02-cdrive.E01` | rd-02 (Remote Desktop Services host) | Disk image (NTFS C:, EWF/E01) |
| `/mnt/artifacts/srl-2018/base-rd-02-memory.img` | rd-02 (Remote Desktop Services host) | Memory capture (raw) |
| `/home/sansforensics/artifacts/srl2018/base-file-cdrive.E01` | file (file server) | Disk image (NTFS C:, EWF/E01) |
| `/home/sansforensics/artifacts/srl2018/base-file-memory.img` | file (file server) | Memory capture (raw) |
| `/home/sansforensics/artifacts/srl2018/base-wkstn-01-mem.img` | wkstn-01 (workstation) | Memory capture (raw) |
| `/home/sansforensics/artifacts/srl2018/base-wkstn-05-memory.img` | wkstn-05 (workstation) | Memory capture (raw) |
...
investigation's case id** — `SetCaseId("SRL-2018")` — so every tool execution is recorded under that
...
**Audit-trail reviewer.** This case ships a static `reports/report.html` (with `reports/report.js`) — open it in a
