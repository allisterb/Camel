...
public class WebServerAnalysisWorkflow : Workflow
{
        ...
        string accessLogPath, string[]? extraPatterns = null, int maxFindings = 100)
    {
        ...
        var lines = (await DiskAnalysis.GrepLinesAsync(accessLogPath, patterns, ignoreCase: true, maxMatches: hitCap)).Result;
        if (lines is null)
        ...
        string webRoot, string? rulesFile = null)
    {
        ...
        var matches = (await Yara.ScanAsync(rulesFile, webRoot, new YaraOptions { Recurse = true, Timeout = 120 })).Result;
        if (matches is null)
