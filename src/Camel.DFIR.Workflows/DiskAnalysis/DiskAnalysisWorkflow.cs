...
public partial class DiskAnalysisWorkflow : Workflow
{
    ...
    public async Task<WorkflowResult<EwfImageMount>> MountEwfImageAsync(string imageFile, string mountDir)
    {
        ...
        var info = (await DiskAnalysis.EwfInfoAsync(imageFile)).Result;
        if (info is null)
        ...
        var partitionTable = (await DiskAnalysis.MmlsAsync(rawDevice)).Result;
    ...
    public async Task<WorkflowResult<FileSystemMount>> MountFileSystemAsync(EwfImageMount imageMount, int offset, string? mountDir = null)
    {
        ...
        var fs = (await DiskAnalysis.FsStatAsync(imageMount.RawDevice, offset)).Result;
    ...
    public async Task<WorkflowResult<ImageVerification>> VerifyImageAsync(string imageFile)
    {
        ...
        var verify = (await DiskAnalysis.EwfVerifyAsync(imageFile)).Result;
        if (verify is null)
        ...
        string imageFile, int? offset = null, string timezone = "UTC", string? bodyfilePath = null)
    {
        ...
        var fs = (await DiskAnalysis.FsStatAsync(imageFile, offset)).Result;
        if (fs is null || string.IsNullOrWhiteSpace(fs.FileSystemType))
        ...
        var entries = (await DiskAnalysis.MactimeAsync(bodyfilePath, timezone)).Result;
        if (entries is null)
        ...
        string imageFile, string outputDir, int? offset = null, bool includeDeleted = true)
    {
        ...
        if (!countR.IsSuccess)
            return WorkflowResult<FileRecovery>.Failure(countR.Message ?? $"tsk_recover failed for '{imageFile}' (offset {offset}).");
        var count = countR.Result;
