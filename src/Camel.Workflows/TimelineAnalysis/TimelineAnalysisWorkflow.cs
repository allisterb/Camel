...
public class TimelineAnalysisWorkflow : Workflow
{
    ...
    /// <see cref="Camel.Toolkits.WindowsAnalysisToolkit.MFTECmdBodyfileAsync"/> or fls), it is appended into the
    ...
    public async Task<WorkflowResult<SuperTimeline>> PivotAroundAsync(string storageFile, DateTimeOffset pivot, int sliceSizeMinutes = 5)
    {
        ...
        var iso = Iso(pivot);
        var events = await Timeline.PsortAsync(storageFile, slice: iso, sliceSize: sliceSizeMinutes);
        if (events is null)
            return WorkflowResult<SuperTimeline>.Failure($"psort failed to slice '{storageFile}' around {iso}.");

        var info = await Timeline.PinfoAsync(storageFile);
        op.Complete();
        ...
        string storageFile, string taggingFile = DefaultWindowsTagFile, string[]? categories = null)
    {
        ...
        var events = await Timeline.PsortAsync(storageFile, filter);
        if (events is null)
        ...
        int sliceSizeMinutes = 5, int maxPivots = 20)
    {
        ...
        {
            var surrounding = await Timeline.PsortAsync(storageFile, slice: Iso(x.Time!.Value), sliceSize: sliceSizeMinutes) ?? [];
    ...
    /// <see cref="Camel.Toolkits.TimelineToolkit.PsortSearchAsync"/>), so it finds hits a psort attribute filter
        ...
        string storageFile, string[] keywords, string? from = null, string? to = null)
    {
        ...
        var matches = await Timeline.PsortSearchAsync(storageFile, pattern, filter);
        if (matches is null)
