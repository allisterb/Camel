...
#
# Every Camel ExecuteJavaScript result ends with an audit handle the agent cites in its report:
#     [audit] case=<caseId> invocation=<id>
# Give this script that invocation id and it prints the invocation boundary plus every command that ran
# under it — with full attribution (workflow > toolkit/op > command line, host, exit code, duration) — from
# the per-case audit log alone. This is the "three-claim trace": finding -> invocation id -> tool execution.
#
# Usage:  ./trace.sh <invocationId> [caseId]
...
inv="${1:?usage: ./trace.sh <invocationId> [caseId]}"
case_id="${2:-srl-2018-rd01}"
file="$(dirname "$0")/audit-${case_id}.clef"
[ -f "$file" ] || { echo "audit file not found: $file" >&2; exit 1; }

echo "Case: ${case_id}   Invocation: ${inv}   (${file})"
echo
jq -r --arg inv "$inv" '
  select(.InvocationId == $inv)
  | if .EventType == "invocation" then
      "• invocation \(.Phase)  \(.["@t"])"
