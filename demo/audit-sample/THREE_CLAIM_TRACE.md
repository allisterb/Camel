...
— three analysis executions through the same workflow → toolkit → environment code paths the MCP server drives.
...
- Every `Execute` result ends with an **audit handle**: `[audit] case=srl-2018-rd01 execution=<id>`.
  The agent cites that `execution` id next to each finding.
- Each command event carries the full chain ambiently: **`Workflow` › `WorkflowOperation` › `Toolkit`/`Operation`
  › `Command`/`Arguments`**, plus `Host`, `ExitCode`, `DurationMs`, `CaseId`, `ExecutionId`.

Trace any finding with the cited execution id:

```bash
./trace.sh <executionId>          # e.g. ./trace.sh 7f3a9c21
```
...
> the **squirreldirectory.com** C2. `[audit] case=srl-2018-rd01 execution=7f3a9c21`"
...
> times) for cross‑referencing against the timeline. `[audit] case=srl-2018-rd01 execution=2e8b14d6`"
...
{"@t":"2026-06-12T23:46:29.40Z","Host":"192.168.8.117",
 ...
 "ExecutionId":"2e8b14d6","CaseId":"srl-2018-rd01"}
```
...
> volume) as the basis for the file‑system timeline. `[audit] case=srl-2018-rd01 execution=9a4f7b03`"
...
{"@t":"2026-06-12T23:46:30.06Z","Host":"192.168.8.117",
 ...
 "ExecutionId":"9a4f7b03","CaseId":"srl-2018-rd01"}
```

The record count traces to the **MFTECmd** execution. (No `Workflow` field here — this execution called the
...
The whole investigation is in one file, grouped by execution. List the headline tool executions (those that ran
...
Each execution also has `execution started`/`completed` boundary events (the `started` event carries the exact
