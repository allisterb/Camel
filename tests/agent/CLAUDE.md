...
1. **`camel-sdk-index`** (`camel://sdk/index`) — the **map**: the execution model and the complete inventory of
   every object (toolkits, workflows, the anomaly engine), every method name and every returned model type, grouped
   by **subject area**.
2. Then, per task, the **areas you touch**: **`camel://sdk/core/{Area}`** for the method detail and
   **`camel://sdk/schema/{Area}`** for the fields of what those methods return. Read the two or three areas the
   objective needs, not all of them (`camel://sdk/core/all` / `camel://sdk/schema/all` serve the whole documents).
...
`Execute` with `log('camel up');`. **Then call the `SetCaseId` tool with a short case id for this
