...
public class PacketAnalysisTests : TestsRuntime
{
    ...
    {
        var info = (await toolkit.CapInfoAsync(Pcap)).Result;
        Assert.NotNull(info);
    ...
    {
        var phs = (await toolkit.ProtocolHierarchyAsync(Pcap)).Result;
        Assert.NotNull(phs);
    ...
    {
        var convs = (await toolkit.ConversationsAsync(Pcap, "tcp")).Result;
    ...
    {
        var eps = (await toolkit.EndpointsAsync(Pcap, "ip")).Result;
        Assert.NotNull(eps);
    ...
    {
        var rows = (await toolkit.FieldsAsync(Pcap, "dns.flags.response==0", ["dns.qry.name", "dns.qry.type"])).Result;
        Assert.NotNull(rows);
    ...
    {
        var stream = (await toolkit.FollowStreamAsync(Pcap, "tcp", 0)).Result;
    ...
    {
        var conns = (await toolkit.TcpTraceAsync(Pcap)).Result;
    ...
    {
        var matches = (await toolkit.NgrepAsync(Pcap, "GET", "tcp port 80")).Result;
    ...
    {
        var fps = (await toolkit.P0fAsync(Pcap)).Result;
        Assert.NotNull(fps);
        Assert.NotEmpty(fps);
    ...
    public async Task CanRunSuricata()
    {
        ...
        try
        {
            var alerts = (await toolkit.SuricataAsync(Pcap, dir)).Result;
