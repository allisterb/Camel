...
public class PacketAnalysisTests : TestsRuntime
{
    ...
    {
        var info = await toolkit.CapInfoAsync(Pcap);
        Assert.NotNull(info);
    ...
    {
        var phs = await toolkit.ProtocolHierarchyAsync(Pcap);
        Assert.NotNull(phs);
    ...
    {
        var convs = await toolkit.ConversationsAsync(Pcap, "tcp");
    ...
    {
        var eps = await toolkit.EndpointsAsync(Pcap, "ip");
        Assert.NotNull(eps);
    ...
    {
        var rows = await toolkit.FieldsAsync(Pcap, "dns.flags.response==0", ["dns.qry.name", "dns.qry.type"]);
        Assert.NotNull(rows);
    ...
    {
        var stream = await toolkit.FollowStreamAsync(Pcap, "tcp", 0);
    ...
    {
        var conns = await toolkit.TcpTraceAsync(Pcap);
    ...
    {
        var matches = await toolkit.NgrepAsync(Pcap, "GET", "tcp port 80");
    ...
    {
        var fps = await toolkit.P0fAsync(Pcap);
        Assert.NotNull(fps);
        Assert.NotEmpty(fps);
    ...
    public async Task CanRunSuricata()
    {
        ...
        try
        {
            var alerts = await toolkit.SuricataAsync(Pcap, dir);
